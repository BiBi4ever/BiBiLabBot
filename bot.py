...
def exchange_command(message):
    ...
    def query_handler(call1):
        if call1.data == 'key':
            bot.send_message(query.message.chat.id, 'Напишите нужный ключ. Учитываются варианты') #(Можете перечислить допустимые слова)
            ...
            bot.send_message(query.message.chat.id, 'Выберите нужный вариант:', reply_markup=keyboard1)
    ...
    def query_handler2(call2):
        if call2.data == 'acid':
            ...
            bot.send_message(query.message.chat.id, 'Выберите нужный вариант:', reply_markup=keyboard2)
